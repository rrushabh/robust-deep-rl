...
class Agent:
    def __init__(
        ...
        self.critic_target = Critic(obs_shape[0], action_shape[0], hidden_dim).to(device)
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        with torch.no_grad():
            ...
            next_actions = self.actor(next_obs, stddev).sample().cpu().numpy()
            next_actions = torch.from_numpy(next_actions).to(self.device).float()
            next_Q_target = self.critic_target(next_obs, next_actions)
            target_Q = reward + discount * next_Q_target
    ...
    def update_actor(self, obs, obs_bc, action_bc, step):
        ...
        dist_bc = self.actor(obs, stddev)
        # bc_loss = F.mse_loss(dist_bc.sample(), action_bc)
        bc_loss = -dist.log_prob(action_bc).sum(-1, keepdim=True).mean()
