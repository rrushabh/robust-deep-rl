...
class Agent:
	def __init__(
		...
		self.set_train(actor=True, encoder=True)
	...
	def policy_update(self, obs, expert_action): # assumes ACN in eval and actor in train
		...
		actor_loss = F.mse_loss(actor_action, expert_action)
	...
	def set_eval(self, actor, encoder):
		if actor:
			self.actor.eval()
		if encoder:
			self.encoder.eval()

	def set_train(self, actor, encoder):
		if actor:
			self.actor.train()
		if encoder:
			self.encoder.train()
	...
	def set_all_eval(self):
		self.set_eval(actor=True, encoder=True)

	def get_model_dicts(self):
		return self.actor.state_dict(), self.encoder.state_dict()
