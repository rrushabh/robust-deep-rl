...
from agent.base_agent_without_acn import Agent
...
class Workspace:
	...
	def model_training_step(self):
		...
		self.agent.set_all_eval()
		self.agent.set_train(actor=True, encoder=False)
	...
	def run(self):
		...
		self.agent.set_train(actor=True, encoder=True)
		...
		for ep_num in iterable:
			...
			if ep_num in self.cfg["rl_checkpoints"]:
				...
				actor_state_dict, encoder_state_dict = self.agent.get_model_dicts()
